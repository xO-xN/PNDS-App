...
- **[The Template repository's creator guide](https://github.com/xO-xN/PNDS-Template/blob/main/docs/creator-guide.md)** (genre division): that one is a follow-along workflow guide for the dev machine, read while you build; this manual is a per-module usage reference for rehearsal and performance — answering questions like "how do seats persist?" or "which CSS variables does theming drive?" when you need the answer now. The help center's [Creator's Guide](../template-guide.md) is the same workflow genre and shares the same division with this manual.
