...
- 文件夹是侧栏顶部的分段控件（switch）：track 撑满行宽，段宽随名字分配，白色 pill 滑动切换；段为 `role="tab"` + roving tabindex，←/→ 切换视图；
...
- 导入入口共三处：Welcome 屏中央 accent 主按钮、工程列表末尾居中的 ghost 按钮、⌘O——三者走同一 `promptOpenProject` 流程。
...
- ⌘M 为静音切换（菜单加速器抢占系统隐藏键），⌘←/⌘→ 以 12.5% 步进调节——两者与滑杆共用同一门控与命令路径。
