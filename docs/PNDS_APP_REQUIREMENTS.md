...
五个小点按**位置和出现顺序**对应下列真实事件，颜色不代表阶段：

| 点  | 真实完成事件                                                                                                   |
| --- | -------------------------------------------------------------------------------------------------------------- |
| 1   | 工程检查完成：信任确认、manifest、路径、依赖与端口 preflight 通过                                              |
| 2   | 音频路径就绪：Internal 的 scsynth 与 App Master Synth 已就绪；External 的 target 已校验；None 已确认不使用音频 |
| 3   | Node score server 已启动                                                                                       |
| 4   | 工程 health payload 的 `status === "ready"` 已确认                                                             |
| 5   | monitor WebView 已完成页面载入并可显示                                                                         |

- App 只能在对应事件实际成功后推进一个点；不得用固定计时假装阶段完成；
- 慢启动时，动画停留在当前点，直到下一个真实事件完成；
- 当前 Logo 中两个背景大圆分别与第 2、4 个点同时进入；该对应关系必须保留；
- 第 5 点完成后才开始当前 Logo 的五点收束与旋转；旋转是成功的短暂结尾，不表示额外启动阶段；
- 旋转完成时使用 dissolve：Logo/loading layer 由不透明淡出，已在底层就绪的 monitor WebView 同时淡入；monitor 不移动、不改变布局；
...
加载阶段由**第几个点**确定，而不是由点的颜色确定。Retry 创建新的 loading session，可以重新随机配色。
...
具体的 Tauri/React ↔ p5 控制 API、动画时长与 WebView 承载机制留待实际开发时确定；不得改变上述阶段、随机性和转场语义。
