...
运行中选卡完全自由（v1.2.3 #39）：点击工程卡、⌘1..9、⌘O、Finder 拖放与双击 .pnds 五条路径统一为"选中 + preflight"，不弹确认框、不重置 session、主视图不从监视页掉回欢迎页；卡片左缘运行竖线与文件夹"使用中"圆点跟随 session 所属工程（与选中无关），白色选中 pill 独立跟随选中，二者可并存于不同卡；被选工程的 preflight 结果（校验中/错误）当场显示在卡上。"切换工程必须确认"的契约保留，但确认时机移至启动动作：按下 Load 且存在 live session 时确认"将先关闭正在运行的工程"（随 v1.2.3 设置卡批次交付）。restart 过程中必须保留当前工程选中状态和待应用设置，不得让侧栏无故取消选择。
