...
（输出设备自 v1.4.0 #58 起是设置面板「音频」栏的全局偏好：运行中锁定、下次启动生效，不再走 Change/restart。）
...
- 设备选择是本机偏好（v1.4.0 #58）：运行中锁定，修改在下次工程启动生效（运行契约 §6/§7.2）；设备能力查询失败不再前置拦截 Load——启动路径在后端完成权威解析并干净失败；
