...
- Rust（`help.rs` tests）：两树 markdown 文件集合相等且等于 `HELP_DOCUMENTS` 登记；`tauri.conf.json` 逐文件覆盖两树（含陈旧映射检查）；资源路径缺失则 tauri 构建脚本先一步失败。
- 前端（`help-trees.test.ts`，`import.meta.glob` 实读两树）：两树文件集合相等；en 树每篇恰一个 `#` 标题、锚点 parity 成立、**树内链接全部在 en 树内可解析**（复用 `resolveHelpLink`）。
...
1. 文件落进**两棵树**（`docs/zh-CN/…` 与 `docs/en/…`，路径同名），以 `#` 标题开头；
2. `tauri.conf.json` 的 `bundle.resources` 各加一行映射（`help/<tree>/…`）；
3. `help.rs` 的 `HELP_DOCUMENTS` 加 `(id, 路径)`（两树同构，一份清单即可）；
4. `help-corpus.ts` 的 `HELP_BOOKS` 把 id 归册定位；
5. `help-scale.test.ts` 补一个 `?raw` import 并在 `DOC_PATHS` 登记。

五处都改齐，`check:all`（两侧漂移测试 + 两树集合相等）会替你查漏；`help-trees.test.ts` 走 glob，新文件无需登记即被两树相等断言覆盖。
