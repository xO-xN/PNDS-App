...
- Android 对 `.local` 的 mDNS 解析（浏览器内）实现不一，兼容性待真机验证——用自定义地址面向 Android 演奏者时，务必在演出用的真机上提前扫码验证，不通则回落 IP 注入。该项是 v1.4.0 发布矩阵的一道门（`docs/developer/releases.md`「发布前人工核验」第 4 条），真机结论回写本条。
