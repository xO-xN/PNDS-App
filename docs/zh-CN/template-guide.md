...
- **AI 驱动开发**  
  将工程目录导入你的 AI-Coding Agent，通过对话引导 AI 理解乐谱的交互逻辑、音频/视觉渲染规则以及通信协议，快速编写或重构代码。工程自带的 `AGENTS.md` 会替 agent 取读平台契约与模块手册，无需手动投喂文档。对刚从模板创建的新工程，对 agent 说**「开始」**即可——它会按工程内置的 `docs/start.md` 完成初始化（作品名 / 作者 / 简介就位、版本归零、安装与测试跑绿）。
- **实时加载与预览**  
