...
App 有四台主题（lavender / sand / stage / brutal）。主题跟随让 monitor 页跟着 App 当前主题换色，而 performer 页保持工程自己的配色。
...
App 在 iframe 加载、主题切换、窗口重新聚焦时都会重推——**最新值胜出，应用必须幂等**（同一份 palette 应用多少次都落在同一状态）。未知或畸形消息被静默忽略，页面绝不因它报错。
...
首帧：URL 带 `?theme=<name>` 时，脚本用内置的四份 palette 先画第一帧（App 目前不发这个参数，是给独立预览留的口子）；App 消息到达后逐字覆盖。
