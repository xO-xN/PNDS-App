// Multichannel Tone Test integration tests: manifest contract, HTTP routes, health
// payload, fader API validation, and graceful shutdown. These start the
// real server in "external-osc" mode (no scsynth, no audio device) so the
// suite runs anywhere without audio hardware.
...
test('manifest satisfies the score-project contract', () => {
  ...
  assert.equal(manifest.id, 'multichannel-tone-test')
...
test('manifest passes the App preflight without node_modules', () => {
  ...
  // Preflight rule (spec §2): zero production deps -> no node_modules needed.
  assert.ok(!fs.existsSync(path.join(PROJECT_ROOT, 'node_modules')))
...
test('performer health reports ready with the runtime contract payload', async () => {
  ...
  try {
    ...
    assert.equal(health.projectId, 'multichannel-tone-test')
...
test('performer / is a no-UI stub, monitor serves the fader page', async () => {
  ...
  try {
    ...
    assert.match(monitorHtml, /Multichannel Tone Test/)
